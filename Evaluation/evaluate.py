...
Usage (example):
python evaluate.py --csv example_data.csv --model-name llama3.2 --mlflow-experiment ner --mlflow-system-prompt-id NER_System/1 --mlflow-user-prompt-id NER_User/1
...
import pandas as pd
import mlflow
from LlmKnowledgeGraph.InferenceApi.entityInference import EntityInferenceProvider, get_unique_entity_names
...
def evaluate_dataset(
    ...
    with mlflow.start_run(run_name=f"GPU-ENTITY-ONLY-V2-{model_name}-{user_prompt_id}-{system_prompt_id}"):
